...
func runArtifact(args []string, stdout, stderr io.Writer) int {
	...
	case "link":
		...
		if fs.NArg() < 1 {
			fmt.Fprintln(stderr, "Usage: symroom artifact link <path> [--title ...] [--identity <name>]")
			return int(exitcodes.ExitNoInput)
		}
		filePath := fs.Arg(0)
		...
		if err != nil {
			if errors.Is(err, artifact.ErrOutsideRoot) {
				fmt.Fprintln(stderr, "Error: path is outside artifact root")
				return int(exitcodes.ExitNoInput)
			}
			fmt.Fprintf(stderr, "Error linking artifact: %v\n", err)
	...
	case "unlink":
		...
		if fs.NArg() < 1 {
			fmt.Fprintln(stderr, "Usage: symroom artifact unlink <artifact_id> [--identity <name>]")
			return int(exitcodes.ExitNoInput)
		}
		artID := fs.Arg(0)
		id := resolveIdentity(*idFlag)
		...
		if err != nil {
			fmt.Fprintf(stderr, "Error unlinking artifact: %v\n", err)
			return int(exitcodes.ExitGeneric)
		}
		fmt.Fprintln(stdout, ev.ID)
		return int(exitcodes.ExitOK)
	...
	case "list":
		...
		if err != nil {
			fmt.Fprintf(stderr, "Error listing artifacts: %v\n", err)
			return int(exitcodes.ExitGeneric)
		}
		if *jsonFlag {
